...
for my $item (@inventory) {
    ...
    if (exists $entityInfo{$entityID}) {
        $entityInstance = $entityInfo{$entityID} + 1;
    }

    printDebug("entityID => $entityID , entityInstance => $entityInstance");

    $entityInfo{$entityID} = $entityInstance;
...
sub writeToFile
{
    ...
    print $fh "  "."path: ".$instancePath;
    print $fh "\n";
    print $fh "  "."entityID: ".$fruTypeConfig->{$fruType}->{'EntityID'};
    print $fh "\n";
    print $fh "  "."entityInstance: ".$entityInfo{$fruTypeConfig->{$fruType}->{'EntityID'}};
    print $fh "\n";
    print $fh "  "."interfaces:";
    print $fh "\n";
