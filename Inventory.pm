...
sub removeModuleFromPath
{
    ...
    for my $item (@$inventory) {
        ...
        if ($type eq "PROC") {
            my $godelTarget = $targetObj->getTargetParent($item->{TARGET});
            my $procSocTarget = $targetObj->getTargetParent($godelTarget);
	    ...
	    #Note that the -B isn't always present
  	    ...
  	    $item->{OBMC_NAME} =~ s/\w+-(\d+)\/$name(-)*(\d+)*/$exp*$procSockPosition+$chipPosition/e;
