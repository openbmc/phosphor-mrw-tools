...
printNode($f, 1, "fmc", getBMCSPIFlashNode());
...
#flash (a chip select line is used to switch between them.)
#Will look like:
#  fmc {
#    status = "okay"
#    flash@0 {
    ...
    foreach my $spi (@{$connections->{CONN}}) {
        ...
        if (!$g_targetObj->isBadAttribute($spi->{SOURCE}, "SPI_FUNCTION")) {
            ...
            if ($function eq "BMC_CODE") {
                ...
                $bmcFlash{$flashName}{COMMENT} = "$spi->{SOURCE} ->\n" .
                                                 "$spi->{DEST_PARENT}";

                $bmcFlash{$flashName}{status} = "okay";

                #Add in anything specified in the config file for this chip.
                addBMCFlashConfigProperties(\%{$bmcFlash{$flashName}},
                                            $chipSelect);
