...
sub generateJSONFile
{
    ...
    foreach my $group (sort keys %hashGroup)
    {
        ...
        foreach my $led (sort keys %{ $hashGroup{$group} })
        {
            ...
            foreach my $property (sort keys %{ $hashGroup{$group}{$led}})
            {
                if ($property eq "DutyOn")
                {
                    $dutyOn = $hashGroup{$group}{$led}{$property};
                }

                if ($property eq "Period")
                {
                    $period = $hashGroup{$group}{$led}{$property};
                }
            }

            my $m = member Emp($led, "", $dutyOn, $period, "");
            push @members, $m;
        }
        my $l = led Emp($group, \@members);
        push @leds, $l;
    }
